...
const ExampleComponent = () => {
  ...
  const [currentTask, setCurrentTask] = useState({ title: '', note: '', completed: null });
  ...
  const createOrUpdateTask = () => {
    ...
    const request = currentTask.id
      ? axios.put(`http://localhost:6001/tasks/${currentTask.id}`, currentTask, { headers })
  ...
  const handleDeleteTask = (taskId) => {
    if (window.confirm("Are you sure you want to delete this task?")) {
      axios.delete(`http://localhost:6001/tasks/${taskId}`)
        .then(response => {
  ...
  return (
    <div>
      <h1>Tasks</h1>
      <button className="large" onClick={() => handleOpenModal()}>Create New Task</button>
          ...
          <tbody>
            {tasks.map(task => (
              <tr key={task.id}>
                ...
                <td>
                  <button onClick={() => handleOpenModal(task)} className="btn btn-primary">Edit</button>
                  &nbsp;
